...
# Start SUMO
traci.start([sumo_binary, "-c", "test.sumocfg", "--start"])  # Ensures SUMO stays open
time.sleep(2)  # Wait for SUMO to initialize

# Get all junctions from SUMO
...
# Define distance threshold (meters) for vehicle detection at junctions
...
# Function to label junctions in SUMO GUI using POIs
def label_junction(junction_id, position):
    traci.poi.add(
        poiID=f"label_{junction_id}",  # Unique label for each junction
        x=position[0],
        y=position[1],
        color=(255, 255, 255, 255)  # White label for visibility
    )
...
if gui_mode == "1":
    ...
    for junction in junctions:
        ...
        label_junction(junction, junction_positions[junction])  # Add label
